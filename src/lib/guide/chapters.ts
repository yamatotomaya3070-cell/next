...
export const GUIDE_CHAPTERS: GuideChapter[] = [
    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          { text: "左下の［エフェクト］→［タイトル］で［テキスト］を、字幕の段（V2 か V3）の、最初の音声の頭までドラッグする" },
          { text: "字幕をクリックしてえらび、右上の［インスペクタ］の文字の欄にセリフを入れる" },
          { text: "インスペクタの下のほうで見た目を変える：［フォント］［サイズ］［カラー］で文字を、［位置］の X・Y で場所を、［ストローク］で縁取りを、［ドロップシャドウ］で影を、［背景］で帯を決める", hint: "変えた所は、ビューアで見ながら確かめる" },
          { text: "1つ目ができたら、それをえらんでコピーし、赤い線（再生の位置）を次の音声の頭に動かして貼り付ける。文字だけ変えて、長さを音声に合わせる", hint: "Ctrl + C → 再生位置を動かす → Ctrl + V" },
        ],
        note: {
          kind: "warn",
          title: "見た目は最初の1つで決める",
    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          ...
          { text: "［エフェクト］→［タイトル］→［テキスト］を、テロップの段（字幕より上の段）の、出したい所までドラッグし、右はしを終わりに合わせる" },
          { text: "インスペクタの文字の欄に文字を入れ、［フォント］［サイズ］［カラー］［位置］［ストローク］で指定どおりの見た目にする" },
  ...
  },
  {
    slug: "worker",
    no: 11,
    title: "案件を作るパソコンの準備（職員の方へ）",
    summary:
      "「YouTube動画生成」で案件を作るには、動画を組み立てる「ワーカー」が動いているパソコンが事業所に1台必要です。その1台を準備します。",
    forStaff: true,
    sections: [
      {
        id: "why",
        title: "なぜ必要か",
        lead: "アプリ（URL）は、テーマを「待機中」のジョブとして受け付けるところまでしかしません。",
        steps: [
          { text: "動画の組み立て（音声を作る・場面の映像を描く・素材ZIPを作る・関門を通す・案件を登録する）は、パソコンの中で動く「ワーカー」が行います" },
          { text: "ワーカーが動いているパソコンが1台も無いと、ジョブは「待機中」のまま進みません" },
          { text: "ワーカーはインターネット経由でアプリとつながるので、どのパソコンでも構いません。ただし事業所で1台だけにします" },
        ],
        note: {
          kind: "warn",
          title: "向いているパソコン",
      ...
      },
      {
        ...
        download: { href: "/guide/setup/kizuna_worker_setup.zip", label: "ワーカー セットアップ用ファイルをダウンロード", size: "5KB" },
        steps: [
          ...
          { text: "「セットアップが終わりました」と出たら完了。ワーカーが自動で起動し、タスクバーに「絆ワーカー」の窓が出ます" },
        ],
        note: {
          kind: "tip",
          title: "設定ファイルが無いとき",
      ...
      },
      {
        ...
        title: "動いているか確かめる",
        steps: [
          { text: "タスクバーに「絆ワーカー」という黒い窓が最小化で出ています。閉じないでください（閉じるとワーカーが止まります）" },
      ...
      },
      {
        ...
        title: "こまったとき",
        steps: [
          { text: "「待機中」のまま進まない → ワーカーが動いていません。デスクトップの「絆ワーカーを起動」をダブルクリックするか、パソコンを再起動する" },
